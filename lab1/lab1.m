...
% Edited by AG, 2016: Removed 'erasemode' 'xor'.
...
hold on
graf=plot(X,Y); %graf=plot(X,Y,'erasemode','xor');
...
elseif strcmp(action,'newtonstega'),
 if omstart==1,
  ...
  newtonkurva=plot(newtonpunkter(:,1),newtonpunkter(:,2),'k','linewidth',[5]);  
...
set(fgoldenyruta,'string',num2str(y(4)));  
  ...
  x=[x;x];
  y=[-10 -10 -10 -10;y];
  steg=plot(x,y,'r'); %steg=plot(x,y,'r','erasemode','xor');  
  found=0;
  tol=1e-6;
  while not(found),
   if Flambda<=Fmy
...
elseif strcmp(action,'stega'),
 if omstart==1,
  ...
  cpoint=get(gca,'currentpoint');
   ...
   newtonkurva=plot(newtonpunkter(:,1),newtonpunkter(:,2),'k','linewidth',[2]);   
...
if newtonon
  set(newtonfruta,'string',num2str(newtonf));
   ...
   modnewtonkurva=plot(modnewtonpunkter(:,1),modnewtonpunkter(:,2),'b','linewidth',[3]);
...
if modnewtonon
  set(modnewtonfruta,'string',num2str(modnewtonf));
   ...
   sdarmijokurva=plot(sdarmijopunkter(:,1),sdarmijopunkter(:,2),'m','linewidth',[4]);
...
if sdarmijoon
  set(sdarmijofruta,'string',num2str(sdarmijof));
   ...
   strategikurva=plot(strategipunkter(:,1),strategipunkter(:,2),'r','linewidth',[5]);
